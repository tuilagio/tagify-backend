...
impl LetsEncrypt {
    ...
    pub fn certificate(&self) -> Result<Option<Certificate>, String> {
        ...
        println!(
            ...
            account_debug.is_status_deactivated()
        );
    ...
    pub fn account(&self) -> Result<Account<FilePersist>, String> {
        ...
        let directory = match if staging {
            ...
            match std::fs::create_dir_all(&staging_dir) {
                ...
                Err(e) => {
                    ...
                    return Err(format!("Could not create staging directory: {:?}", e));
                }
            }
            Directory::from_url(
                FilePersist::new(&staging_dir),
            ...
            match std::fs::create_dir_all(&account_dir) {
                ...
                Err(e) => {
                    info!(
                        "Path is: {}",
                        account_dir.into_os_string().into_string().expect("")
                    );
                    error!("Failed to create directory: {}", e);
                    return Err("Could not create base directory".into());
                }
            }
            info!(
                "Path is: {}",
        ...
        // Todo: From config
        let account = match directory.account("it.networking.logging@gmail.com") {
            Ok(account) => account,
    ...
    fn build_cert(&self) -> Result<(), String> {
        ...
        let ord_cert = match ord_csr.finalize_pkey(pkey_pri, 2500) {
...
pub async fn nonce_request(req: HttpRequest) -> Result<NamedFile> {
