...
impl Oauth {
    ...
    pub fn get_token(&self) -> String {
        let credentials =
            Credentials::from_file(&self.cred_file).expect("Could not read google credential file");
        ...
        let jwt = Jwt::new(
            ...
            credentials
                .rsa_key()
                .expect("Invalid rsa key in credential file"),
            None,
        );
        ...
        let json: serde_json::Value =
            serde_json::from_str(&token).expect("JSON TokenResponse was not well-formatted");
        let token_bearer: String = json
            .get("access_token")
            .expect("TokenResponse should have 'access_token' key")
            .to_string()
            .replace("\"", "");
