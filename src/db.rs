...
use crate::models::{User, Hash, CreateUser};
use crate::errors::{DBError};

pub async fn get_user_by_name(client: deadpool_postgres::Client, username: &str) -> Result<User, DBError>{
...
pub async fn get_user(client: &deadpool_postgres::Client, id: i32) -> Result<User, DBError>{
   ...
   // Query data
   let result = client.query_one("SELECT * FROM users WHERE id = $1", &[&id]).await?;
...
pub async fn update_user(client: &deadpool_postgres::Client, user: &User) -> Result<User, DBError>{
    ...
    if user.password.len() < 4 {
        return Err(DBError::BadArgs{err: "Password is too short".to_owned()});
    }
...
pub async fn create_user(client: &deadpool_postgres::Client, user: &CreateUser) -> Result<User, DBError>{
...
pub async fn delete_user(client: &deadpool_postgres::Client, user_id: i32) -> Result<User, DBError>{
   let result = client.query_one(
